import adressService from '../services/adressService.js'; //model do usuario
...
const findAdress = async (req, res) => {
    ...
    const endereco = await adressService.findOneById(token.id, addressId);
...
const findAdressess = async (req, res) => {
    const usertoken = req.headers.authorization; // recebe o token da sessao
    let token = decoder(usertoken) // decodifica o token
    let enderecosJSON = [];

    let enderecos = await adressService.findAllById(token.id);
...
const deleteAdress = async(req, res) => {  
    ...
    const deleteAdress = await adressService.deleteAdress(addressId, token.id); // exclui o endereco pelo ID no BD
